...
/*   Updated: 2023/01/20 16:33:12 by rrebois          ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */

#include "../incs_bonus/so_long_bonus.h"

static int	check_vil(t_game *game)
{
	size_t	i;
	int		vil;
...
{
	if (game->width + game->height < 9)
		return (ft_printf("Error\nMap too small.\n"), 1);
	if (check_closed_map(game) == 1)
		return (1);
	if (check_items(game) == 1 || check_vil(game) == 1)
	{
		ft_printf("Error\nMap must contain 1 exit, 1 starting point, \
1 ennemy and at least 1 item.\n");
		return (1);
	}
